...
class ash:
    def __init__(
        ...
        if bin_num is None:
            ...
            if len(self.data) >= 40 and not force_scott and kde_result:
                ...
                self.bins_from_bw()
                self.bw2, self.kde_mesh, self.kde_den = kde(
                    self.data, None, self.ash_mesh.min(), self.ash_mesh.max())
            else:
    ...
    def bins_from_bw(self):
        ...
        self.bin_num = np.ceil(
            ((self.data_max - self.data_min) / self.bin_width))
        self.MIN = self.data_min - self.bin_width
    ...
    def calc_ash_den(self, normed=True):
        self.ash_mesh = np.linspace(
            self.MIN, self.MAX, (self.bin_num + 2) * self.shift_num)
        ...
        for i in range(self.shift_num):
            ...
            hist, self.bin_edges = np.histogram(
                self.data, self.bin_num + 1, range=hist_range, normed=normed)
            ...
            self.ash_den = self.ash_den + \
                np.r_[[0] * i, hist_mesh, [0] * (self.shift_num - i)]
            # print(ash_den)
    ...
    def calc_ash_unc(self):
        ...
        self.sigma = np.sqrt(
            np.average(
                (self.ash_mesh - self.mean)**2,
                weights=self.ash_den))
        # print(area, self.unc ,self.sigma)

    def plot_ash_infill(self, ax=None, color='#92B2E7', normed=True):
        ...
        for i in range(self.shift_num):
            ...
            hist, bin_edges = np.histogram(
                self.data, self.bin_num + 1, range=hist_range, normed=normed)
            self.hist_max = max(hist) if max(
                hist) > self.hist_max else self.hist_max
            # gbar(ax, bin_edges[:-1], hist, width=self.bin_width,
            #      alpha=0.5/self.shift_num)
            ax.hist(self.data, self.bin_num + 1, range=hist_range,
                    histtype='stepfilled', alpha=0.75 / self.shift_num,
                    color=color, linewidth=0, normed=True, rasterized=True)
        ymin, ymax = ax.get_ylim()
        ymax -= ymax * 0.1
        self.hist_max = ymax if ymax > self.hist_max else self.hist_max
        # print(self.hist_max)
        ax.set_ylim(0, self.hist_max + self.hist_max * 0.1)
    ...
    def plot_stats(
        ...
        if short:
            stat_string = (label_str +
                           r"$\mathregular{" + "{:.2uSL}".format(mean) + "}$" +
                           "\nN = " + str(self.data_len))
        else:
            stat_string = (label_str +
                           r"$\mathregular{" + "{:.2uL}".format(mean) + "}$" +
                           "\nN = " + str(self.data_len))
        ax.text(
            x,
            y,
