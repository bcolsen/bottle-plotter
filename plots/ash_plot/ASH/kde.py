...
def kde(data, N=None, MIN=None, MAX=None):
    ...
    idx_arr = [iN * iN for iN in xrange(1, N)]
    ...
    try:
        t_star = scipy.optimize.brentq(fixed_point, 0, guess,
                                       args=(M, idx_arr, SqDCTData))
    except ValueError:
        # print('Failed KDE bandwidth selection by data fequency')
        return None
...
def fixed_point(t, M, idx_arr, a2):
    ell_val = 7
    idx_arr = sci.float128(idx_arr)
    M = sci.float128(M)
    a2 = sci.float128(a2)
    f = 2 * sci.pi**(2 * ell_val) * sci.sum(idx_arr**ell_val * a2 *
                                           sci.exp(-idx_arr * sci.pi**2 * t))
    for s in range(ell_val, 1, -1):
        K0 = sci.prod(xrange(1, 2 * s, 2)) / sci.sqrt(2 * sci.pi)
        const = (1 + (1 / 2)**(s + 1 / 2)) / 3
        time = (2 * const * K0 / M / f)**(2 / (3 + 2 * s))
        f = 2 * sci.pi**(2 * s) * sci.sum(idx_arr**s * a2 *
                                          sci.exp(-idx_arr * sci.pi**2 * time))
