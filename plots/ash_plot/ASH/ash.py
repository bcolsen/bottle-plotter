...
class ash:
    def __init__(
        ...
        if bin_num is None:
            ...
            if len(self.data) >= 50 and not force_scott and kde_result:
                ...
                self.bin_width = 2 * \
                    (stats.iqr(self.data) / (len(self.data)**(1 / 3)))
    ...
    def plot_ash_infill(
        ...
        for i in range(self.shift_num):
            ...
            ax_tmp.hist(self.data, self.bin_num + 1, range=hist_range,
                        histtype='stepfilled',
                        alpha=alpha / self.shift_num, color=color,
                        linewidth=0, normed=normed, rasterized=True)
        ...
        plt.close(fig_tmp)
        # self.hist_img = np.array(
        #    fig_tmp.canvas.renderer._renderer).reshape((480,480,4))
        # outs = BytesIO()
