...
cli
'''
data = '-0.38763, 0.80928, 1.5736, -0.19156, -1.2762, 0.012471, 2.7392, -0.14373, 1.5309, -0.71012, 2.6883, \
             -0.97024, -0.18379, 0.39052, 0.89383, -0.28856, -0.82227, -1.2461, 2.8595, 0.50082'

#==============================================================================
# parser = argparse.ArgumentParser()
# parser.add_argument("test", help="Test label")
# parser.add_argument("thick", type=float, help="As-spun polymer thickness in nm")
# #parser.add_argument("--no_wait", help="Starts anneal time as soon as solvent flow starts",action="store_true")
# parser.add_argument("--type", type=str, choices=test_type_list, help="Type of test")
# parser.add_argument('--step', type=float,  action='append', nargs=3, metavar = ('swell', 'dwell', 'rate'), 
#                         help="Takes 3 floats: Desired swelling ratio, Anneal dwell time in seconds, ramp rate in swelling ratio per minute. \
#                         If you need multiple steps define --step multiple times")
#==============================================================================
...
#    ax.text(0.05, 0.95,'{:.2f}'.format(np.corrcoef(a_vals,b_vals)[0,1]*100) , color='#4C72B0', ha='left', va='top', transform=ax.transAxes, size=16)
#    
...
#    ax.text(0.95, 0.95,'{:.2f}'.format(np.corrcoef(a_vals,b_vals)[0,1]*100) , color='#D95319', ha='right', va='top', transform=ax.transAxes, size=16)
#    ax.set_title(b+'-'+a)
